...
<body>
    ...
    <form method='POST' action="{{route('registro-Admin')}}" autocomplete="off" class="form">
    ...
    <table class="table table-dark table-striped">
        ...
        <tr>
            ...
            <td>{{$admin->email}}</td>
            <td>
                <a href="/admin/deleteAdmin/{{$admin->id}}" class='btn btn-danger'>Eliminar</a>
            </td>
            <td>
                <a class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal{{$admin->id}}">Actualizar</a>
            </td>
        </tr>
        ...
        <div class="modal-dialog">
            ...
            @endif
            <form method='POST' action="/admin/editAdmin/{{$admin->id}}" autocomplete="off">
                @method('PUT');
                @csrf
            <div class="form-group">
                <label for="exampleInputEmail1" class='text-dark'>Nombre</label>
                <input type="text" class="form-control" id="exampleInputEmail1"  value="{{$admin->nombre}}" name="nombre" aria-describedby="emailHelp" placeholder="Nombre">
            </div>
            <div class="form-group">
                <label for="Apellido" class='text-dark'>Apellido</label>
                <input type="text" class="form-control" id="Apellido" value="{{$admin->apellido}}" name="apellido" placeholder="Apellido">
            </div>
            <div class="form-group">
                <label for="correo" class='text-dark'>Email</label>
                <input type="email" class="form-control" id="correo"  value="{{$admin->email}}" name="correo" placeholder="Correo">
            </div>
            </div>
